---
description: 'Agent 循环的工程化：上下文压缩与取舍、注意力与状态回注、工具返回值设计、Hook 拦截、子 Agent 派发、前缀缓存。从 50 行最小循环到生产级 AI 引擎的设计决策。'
head:
  - - meta
    - property: og:description
      content: 'Agent 循环、上下文压缩、工具返回值设计、子 Agent、前缀缓存'
---
...
<figure class="book-fig">
  <img src="/img/fig-3-1.svg" alt="左侧是规划者、执行者、校验者串成的多组件流程图；右侧是模型决策与执行工具构成的循环，循环下方挂着一个形状完全相同的子 Agent 循环">
  <figcaption>图 3-1　编排没有被删除，而是从架构图搬进了工具清单。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-2.svg" alt="左右两块等大的面板，左侧列出多组件架构里工程落在结构上的四项，右侧列出循环架构里工程落在输入上的四项，中间一根粗箭头标注同一批工程量">
  <figcaption>图 3-2　工程量没有减少，只是从结构搬到了输入。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-3.svg" alt="一幅简洁的卡通剖面图：一间没有窗户、房门关闭的房间内坐着一个正在阅读和书写纸条的人，标注为大语言模型；门下只有一条缝，纸条从这里双向传递；门外站着一名助手，旁边是文件、终端和网络等外部工具。助手按照纸条执行操作，再把结果写在新纸条上递回房间；房间里的人看不到外部世界，助手不替他作决定">
  <figcaption>图 3-3　模型只看到上下文里的消息；循环负责执行工具，并把外部世界的结果送回来。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-4.svg" alt="最小 Agent 的结构图：中央是一个闭环，模型接收由系统提示词和消息历史组成的输入，输出文本或工具调用；工具调用进入工具执行器，工具执行器连接 list_files 与 read_file 两个工具，并把结果追加到消息历史后送回模型。图侧用四个清晰标签标出组成部分：一个模型、一段系统提示词、两个工具、一个循环；循环外没有规划者、审核者或其他 Agent 组件">
  <figcaption>图 3-4　最小 Agent 只有四样东西：模型提供判断，提示词规定角色，工具连接外部世界，循环让三者持续工作。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-5.svg" alt="横轴是轮次，纵轴是 token 用量，一条只涨不跌的曲线逼近标着窗口上限的虚线，撞线处标着任务中断">
  <figcaption>图 3-5　历史只增不减，任务越长，离上限越近。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-6.svg" alt="一叠消息从顶部起被逐条置灰停发，第一条置灰的正是标红的用户任务指令，下方保留的消息上画着一个问号">
  <figcaption>图 3-6　滑动窗口按新旧取舍，最先停发的恰恰是任务本身。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-7.svg" alt="两级示意：第一级中若干工具结果块被替换为细小的占位条，骨架保留；第二级中大段对话被整体替换为一块摘要，仅两端原文保留">
  <figcaption>图 3-7　轻的一级替换工具结果，保留骨架；重的一级替换骨架本身。</figcaption>
</figure>

压缩完，任务继续，历史再涨，涨到线再压。整个任务期间，上下文的用量曲线是一条锯齿：涨、压、再涨、再压。

<figure class="book-fig">
  <img src="/img/fig-3-8.svg" alt="横轴是轮次，纵轴是 token 用量，曲线多次涨到阈值线后回落，轻级压缩回落幅度小，重级回落到较低水位，形成高低错落的锯齿">
  <figcaption>图 3-8　压缩让上下文用量变成锯齿曲线，任务不再受单次窗口长度的限制。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-9.svg" alt="横轴是关键信息在上下文中的位置（开头到结尾），纵轴是回答准确率，曲线呈 U 形，两端高中间低；另有一条整体更低的虚线代表更长的上下文">
  <figcaption>图 3-9　同一条信息，放的位置不同，模型的利用率不同（Lost in the Middle, 2023）。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-10.svg" alt="一叠消息的示意，任务目标在顶部被标为灰色远景，末尾紧贴生成位置处插入高亮的任务状态块，一个箭头标注模型注意力的落点">
  <figcaption>图 3-10　复诵把任务状态搬回注意力的落点，而不是指望模型回头看。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-11.svg" alt="三层结构图：对话历史标注自动堆积、会被压缩；工作记忆标注随任务生灭、按条件回注、从不压缩；长期记忆标注跨任务存在、指向第七章">
  <figcaption>图 3-11　三层记忆：流水账、任务状态、长期沉淀，生命周期各不相同。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-12.svg" alt="同一个失败分出两条路径：上方裸报错引出猜测、换参数重试、再失败的循环，标注消耗多轮；下方带方向的报错直接引出一步修正，标注一轮完成">
  <figcaption>图 3-12　报错的措辞决定失败之后要走多少弯路。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-13.svg" alt="左栏是模型上下文里的文件，右栏是磁盘上的文件，起初完全一致；一次批量写入三处修改、两处成功一处失败之后，磁盘已经改变而上下文里仍是旧版本，模型并不知道；中间的返回文本是唯一能把两边重新拉回一致的通道">
  <figcaption>图 3-13　写操作之后，模型上下文里的文件与磁盘上的文件可能错位，返回文本是唯一的对齐通道。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-14.svg" alt="Agent 主循环的两种异常及干预位置：模型返回工具调用时进入工具执行，停滞检测器读取最近的调用历史，并把检测结果附加到最新工具结果；模型没有返回工具调用时进入退出检查，工作记忆仍有未完成项则注入任务状态并继续，状态不足时交给观察者或用户判断，确认完成后才退出">
  <figcaption>图 3-14　循环停滞从执行历史中检测，提前退出由工作记忆、验收条件、观察者或用户判断。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-15.svg" alt="模型提出工具调用后，调用先经过执行前 Hook；Hook 可以放行、修改参数或拒绝。放行后执行工具并经过执行后 Hook，拒绝时直接生成说明原因的工具结果；两条路径最终都把结果写回上下文，进入模型下一轮">
  <figcaption>图 3-15　Hook 介入一次工具调用的过程：在执行前检查，在执行后处理，并把结果写回模型上下文。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-16.svg" alt="左侧展示没有 Hook 时，订单查询、退款条件和审批检查都以条件分支写入 Agent 主循环；右侧展示引入 Hook 后，Agent 内核只保留模型调用、Hook 调度、工具执行和结果写回，退款业务在内核之外维护订单状态、退款条件和审批规则，并通过 Hook 介入退款工具的执行">
  <figcaption>图 3-16　退款规则仍由业务模块维护，只通过 Hook 进入工具执行流程，不进入 Agent 内核。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-17.svg" alt="左侧是主 Agent 的工具清单，read、search 和 runSubAgent 三个工具并列；主 Agent 选择 runSubAgent 后，工具内部启动一个拥有独立上下文的 Agent 循环，任务说明成为子 Agent 的第一条消息；子 Agent 完成后返回结果文本，该文本作为普通工具结果写回主 Agent 的上下文">
  <figcaption>图 3-17　对子 Agent 的派发仍是一次普通工具调用；特殊之处只在于，这个工具内部运行了另一个 Agent 循环。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-18.svg" alt="左侧展示适合拆分的调研任务：子 Agent 在独立上下文中搜索和读取三十个文件，最后只向父 Agent 返回一页带证据的结论，大量中间信息被隔离；右侧展示不适合拆分的代码讨论：子 Agent 读取文件后只返回摘要，用户继续追问具体代码时，父 Agent 因缺少原始细节不得不重新读取文件">
  <figcaption>图 3-18　当中间材料很多而主任务只需要结论时，隔离是收益；当后续决策仍依赖原始细节时，隔离会变成重复获取信息的成本。</figcaption>
</figure>
...
<figure class="book-fig">
  <img src="/img/fig-3-19.svg" alt="上下两条横向 token 序列。上面一条：前面一长段系统提示词与工具定义标为深色，标注已缓存、可复用；后面一小段对话与状态回注标为浅色，标注每轮重新计算。下面一条：同样的序列，中间某个 token 变红，红点之后所有 token 变灰，标注从改动处起全部失效；红点之前仍保持深色，标注前缀不变、仍可复用">
  <figcaption>图 3-19　缓存复用的是前缀；中间一改，后面全部失效。</figcaption>
</figure>
