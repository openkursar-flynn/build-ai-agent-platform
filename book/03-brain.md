...
<figure class="book-fig">
  <img src="/img/fig-3-1.png" alt="左侧是规划者、执行者、校验者串成的多组件流程图；右侧是模型决策与执行工具构成的循环，循环下方挂着一个形状完全相同的子 Agent 循环">
...
<figure class="book-fig">
  <img src="/img/fig-3-2.png" alt="左右两块等大的面板，左侧列出多组件架构里工程落在结构上的四项，右侧列出循环架构里工程落在输入上的四项，中间一根粗箭头标注同一批工程量">
...
<figure class="book-fig">
  <img src="/img/fig-3-3.png" alt="一幅简洁的卡通剖面图：一间没有窗户、房门关闭的房间内坐着一个正在阅读和书写纸条的人，标注为大语言模型；门下只有一条缝，纸条从这里双向传递；门外站着一名助手，旁边是文件、终端和网络等外部工具。助手按照纸条执行操作，再把结果写在新纸条上递回房间；房间里的人看不到外部世界，助手不替他作决定">
...
<figure class="book-fig">
  <img src="/img/fig-3-4.png" alt="最小 Agent 的结构图：中央是一个闭环，模型接收由系统提示词和消息历史组成的输入，输出文本或工具调用；工具调用进入工具执行器，工具执行器连接 list_files 与 read_file 两个工具，并把结果追加到消息历史后送回模型。图侧用四个清晰标签标出组成部分：一个模型、一段系统提示词、两个工具、一个循环；循环外没有规划者、审核者或其他 Agent 组件">
...
<figure class="book-fig">
  <img src="/img/fig-3-5.png" alt="横轴是轮次，纵轴是 token 用量，一条只涨不跌的曲线逼近标着窗口上限的虚线，撞线处标着任务中断">
...
<figure class="book-fig">
  <img src="/img/fig-3-6.png" alt="一叠消息从顶部起被逐条置灰停发，第一条置灰的正是标红的用户任务指令，下方保留的消息上画着一个问号">
...
<figure class="book-fig">
  <img src="/img/fig-3-7.png" alt="两级示意：第一级中若干工具结果块被替换为细小的占位条，骨架保留；第二级中大段对话被整体替换为一块摘要，仅两端原文保留">
...
<figure class="book-fig">
  <img src="/img/fig-3-8.png" alt="横轴是轮次，纵轴是 token 用量，曲线多次涨到阈值线后回落，轻级压缩回落幅度小，重级回落到较低水位，形成高低错落的锯齿">
...
<figure class="book-fig">
  <img src="/img/fig-3-9.png" alt="横轴是关键信息在上下文中的位置（开头到结尾），纵轴是回答准确率，曲线呈 U 形，两端高中间低；另有一条整体更低的虚线代表更长的上下文">
...
<figure class="book-fig">
  <img src="/img/fig-3-10.png" alt="一叠消息的示意，任务目标在顶部被标为灰色远景，末尾紧贴生成位置处插入高亮的任务状态块，一个箭头标注模型注意力的落点">
...
<figure class="book-fig">
  <img src="/img/fig-3-11.png" alt="三层结构图：对话历史标注自动堆积、会被压缩；工作记忆标注随任务生灭、按条件回注、从不压缩；长期记忆标注跨任务存在、指向第七章">
...
<figure class="book-fig">
  <img src="/img/fig-3-12.png" alt="同一个失败分出两条路径：上方裸报错引出猜测、换参数重试、再失败的循环，标注消耗多轮；下方带方向的报错直接引出一步修正，标注一轮完成">
...
<figure class="book-fig">
  <img src="/img/fig-3-13.png" alt="左栏是模型上下文里的文件，右栏是磁盘上的文件，起初完全一致；一次批量写入三处修改、两处成功一处失败之后，磁盘已经改变而上下文里仍是旧版本，模型并不知道；中间的返回文本是唯一能把两边重新拉回一致的通道">
...
<figure class="book-fig">
  <img src="/img/fig-3-14.png" alt="Agent 主循环的两种异常及干预位置：模型返回工具调用时进入工具执行，停滞检测器读取最近的调用历史，并把检测结果附加到最新工具结果；模型没有返回工具调用时进入退出检查，工作记忆仍有未完成项则注入任务状态并继续，状态不足时交给观察者或用户判断，确认完成后才退出">
...
<figure class="book-fig">
  <img src="/img/fig-3-15.png" alt="模型提出工具调用后，调用先经过执行前 Hook；Hook 可以放行、修改参数或拒绝。放行后执行工具并经过执行后 Hook，拒绝时直接生成说明原因的工具结果；两条路径最终都把结果写回上下文，进入模型下一轮">
...
<figure class="book-fig">
  <img src="/img/fig-3-16.png" alt="左侧展示没有 Hook 时，订单查询、退款条件和审批检查都以条件分支写入 Agent 主循环；右侧展示引入 Hook 后，Agent 内核只保留模型调用、Hook 调度、工具执行和结果写回，退款业务在内核之外维护订单状态、退款条件和审批规则，并通过 Hook 介入退款工具的执行">
...
<figure class="book-fig">
  <img src="/img/fig-3-17.png" alt="左侧是主 Agent 的工具清单，read、search 和 runSubAgent 三个工具并列；主 Agent 选择 runSubAgent 后，工具内部启动一个拥有独立上下文的 Agent 循环，任务说明成为子 Agent 的第一条消息；子 Agent 完成后返回结果文本，该文本作为普通工具结果写回主 Agent 的上下文">
...
<figure class="book-fig">
  <img src="/img/fig-3-18.png" alt="左侧展示适合拆分的调研任务：子 Agent 在独立上下文中搜索和读取三十个文件，最后只向父 Agent 返回一页带证据的结论，大量中间信息被隔离；右侧展示不适合拆分的代码讨论：子 Agent 读取文件后只返回摘要，用户继续追问具体代码时，父 Agent 因缺少原始细节不得不重新读取文件">
...
<figure class="book-fig">
  <img src="/img/fig-3-19.png" alt="上下两条横向 token 序列。上面一条：前面一长段系统提示词与工具定义标为深色，标注已缓存、可复用；后面一小段对话与状态回注标为浅色，标注每轮重新计算。下面一条：同样的序列，中间某个 token 变红，红点之后所有 token 变灰，标注从改动处起全部失效；红点之前仍保持深色，标注前缀不变、仍可复用">
